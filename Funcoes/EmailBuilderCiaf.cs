...
namespace nfecreator
{
    class EmailBuilderCiaf
    {
        ...
        public sealed class EmailBuilder
        {
            ...
            public EmailBuilder AdicionarReply(string email, string nome = "")
            {
                //if (!EmailValido(email))
                //    throw new ArgumentException("E-mail do destinatário é inválido!");

                _replay.Add(new MailAddress(email, nome, System.Text.Encoding.UTF8));
                return this;
            }
            ...
            public void Enviar(MailMessage mensagemPredefinida = null)
            {
                try
                {
                    ...
                    //_destinatarios.ForEach(mensagem.To.Insert);
                    foreach (var address in destinatarioss.Split(new[] { ";" }, StringSplitOptions.RemoveEmptyEntries))
                    {
                        mensagem.To.Add(address);
                    }
                    _copia.ForEach(mensagem.Bcc.Add);

                    _replay.ForEach(mensagem.ReplyToList.Add);
            ...
            private void OnDepoisDeEnviarEmail()
            {
                ...
                //   System.Windows.MessageBox.Show("E-mail encaminhado com sucesso.","E-mail encaminhado!", MessageBoxButton.OK, MessageBoxImage.Information);
            }
        }
    }
}
