...
namespace nfecreator
{
    class VendaSatI
    {
        ...
        public List<VendaSatI> GetTodas(int nrvenda)
        {
            List<VendaSatI> lista = new List<VendaSatI>();
            VendaSatI ivenda;
            try
            {
                DbfBase ebase = new DbfBase();
                string instrucao = @"SELECT nrvenda, codigob, descricao, unidade, quantidade, Cast(vunitario As NUMERIC(11,4)) as vunitario, " +
                ...
                "tipol, cstipi, vipi, cdpresta, vcompresta " +
                //"" +
                "from " + ebase.Path + @"\SATCEI.dbf WHERE nrvenda = " + nrvenda + " ";
        ...
        public void UpdateVmimposto(string vmimposto)
        {
            ...
            {
                DbfBase ebase = new DbfBase();
                String instrucao = @"Update " + ebase.Path + @"\SATCEI.dbf SET vmimpostos = " + vmimposto.Replace(",",".") +
