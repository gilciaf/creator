...
namespace nfecreator
{
    class VendaSatce
    {
        ...
        public VendaSatce(int id)
        {
            try
            {
                DbfBase ebase = new DbfBase();
                string instrucao = @"SELECT * from " + ebase.Path + @"\SATCe.dbf WHERE nrvenda = " + id + " ";
        ...
        public void Update()
        {
            ...
            {
                DbfBase ebase = new DbfBase();
                String instrucao = @"Update " + ebase.Path + @"\SATCe.dbf SET statusnfe = '" + Status + "' " +                   
