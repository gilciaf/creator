...
namespace nfecreator
{
    class Vendanfcea
    {
        ...
        public Vendanfcea(int id)
        {
            try
            {
                DbfBase ebase = new DbfBase();
                string instrucao = @"SELECT * from " + ebase.Path + @"\NFCEA.dbf WHERE nrvenda = " + id + " ";
        ...
        public void Update()
        {
            try
            {
                ...
                DbfBase ebase = new DbfBase();
                String instrucao = @"Update " + ebase.Path + @"\NFCEA.dbf SET statusnfe = '" + Statusnfe + "', " +
                    "ambiente = '" + Ambiente + "', " +
                    "chave = '" + chave + "', " +
                    "xml = '" + xml + "', " +
                    "danfe = '" + danfe + "', " +
        ...
        public void Insert()
        {
            ...
            {
                DbfBase ebase = new DbfBase();
                String instrucao = @"Insert into " + ebase.Path + @"\NFCEA.dbf  (nrvenda, statusnfe, ambiente, chave, xml, danfe, merro, nprotocolo )  VALUES (" + nrvenda + @", '" + statusnfe + @"', '', '', '', '', '', ''  )";
