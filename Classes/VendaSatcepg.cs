...
namespace nfecreator
{
    class VendaSatcePG
    {
        ...
        public List<VendaSatcePG> GetItensdePG(int nrvenda)
        {
            List<VendaSatcePG> lista = new List<VendaSatcePG>();
            VendaSatcePG ivenda;
            try
            {
                DbfBase ebase = new DbfBase();
                string instrucao = @"SELECT * from " + ebase.Path + @"\SATCEPG.dbf WHERE nrvenda = " + nrvenda + " ";
